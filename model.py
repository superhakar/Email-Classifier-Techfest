...
def train(ngram,n_estimators,embedding_dim,vocab_size,max_length,num_epochs):
  ...
  classifier = RandomForestClassifier(oob_score=True,n_estimators=n_estimators)
  classifier.fit(X,y)
...
def test():
  ...
  y_pred1 = loaded_model.predict_proba(X_test1)
  ...
  y_pred = y_pred1*0.5 + y_pred2*0.5
